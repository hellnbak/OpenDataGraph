...
2. **Tenant boundary** maps each API-key, service account, or validated OIDC principal to one tenant and applies that tenant to every data lookup and mutation.
...
10. **Relationship and lineage layer** stores indexed directed tenant-scoped graph edges, OpenLineage events, bounded multi-hop traversal, path explanations, synchronous export, and asynchronous integrity-checked export jobs.
...
13. **Identity plane** validates configured OIDC providers with cached discovery; manages tenant-scoped SCIM users, groups, Bulk requests, and durable deprovisioning; and issues hashed, rotating service-account credentials.
14. **Governance operations** unify policy and evidence reviews, deadlines, assignment, SLA reporting, notifications, ownership attestations, and remediation.
...
16. **Operations layer** provides Alembic migrations, backups, readiness, Prometheus metrics, JSON logs, optional OTLP traces, deterministic benchmarks, and read-only soak tooling.
...
API replicas ---- PostgreSQL
      ...
      |               |
 OpenSearch       connectors ---- graph export store / approved S3 sinks
      |
...
PostgreSQL is recommended for shared deployments and concurrent workers. SQLite supports local development, tests, and single-worker evaluation. Alembic owns shared schema upgrades. OpenSearch is a rebuildable metadata index. Evidence and graph-export bytes live in configured local or S3-compatible object storage; the database stores metadata, state, and digests.

## Trust boundaries

Source credentials remain outside catalog and job tables. Synchronous tokens are request-scoped. Queued connectors and webhook integrations resolve secret references only inside workers after endpoint validation. API keys, service accounts, and validated OIDC tokens carry role and tenant context. Service-account verifiers are salted and clear keys are one-time responses. OIDC discovery and discovered JWKS hosts are bounded by provider configuration. SCIM uses a dedicated tenant-bound credential. Graph export sinks require explicit bucket allowlisting and workload identity. Production ingress must terminate TLS, restrict `/metrics`, manage secrets externally, and isolate connector, integration, and export egress.
