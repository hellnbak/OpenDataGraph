...
Standard controls include `tenant-context`, and policy audits remain in the authenticated tenant. v1.7 retains versioned bundles, structured diffs, delegated review, activation, rollback, expiring exceptions, renewal, unified operational review tasks, analytics, and signed metadata-only evidence packaging. See [Policy governance](POLICY_GOVERNANCE.md).
