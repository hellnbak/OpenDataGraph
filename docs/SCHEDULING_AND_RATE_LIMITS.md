...
OpenDataGraph v1.6 runs tenant-scoped connector scans and ownership campaigns on fixed intervals or five-field cron calendars. Workers atomically claim due schedules and enqueue reference-only `connector.scan` or `ownership.campaign.launch` jobs.
