...
Public-link permissions are not evaluated in v1.3. `public_access` remains false and `metadata.public_access_evidence` is `not-evaluated`; this is not proof that an item is private.
